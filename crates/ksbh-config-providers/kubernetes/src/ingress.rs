...
mod tests {
    ...
    #[test]
    fn test_extract_module_names_single_module() {
        let mut annotations = std::collections::BTreeMap::new();
        annotations.insert(
            ksbh_core::constants::KSBH_ANNOTATION_KEY_MODULES.to_string(),
            "rate-limit".to_string(),
        );

        let result = extract_module_names(Some(&annotations));

        assert_eq!(result.len(), 1);
        assert_eq!(&*result[0], "rate-limit");
    }

    #[test]
    fn test_extract_module_names_multiple_modules() {
        let mut annotations = std::collections::BTreeMap::new();
        annotations.insert(
            ksbh_core::constants::KSBH_ANNOTATION_KEY_MODULES.to_string(),
            "rate-limit, oidc,  robots-txt".to_string(),
        );
        ...
        assert_eq!(&*result[0], "rate-limit");
        assert_eq!(&*result[1], "oidc");
        assert_eq!(&*result[2], "robots-txt");
    }

    #[test]
    fn test_extract_module_names_empty_annotations() {
    ...
    #[test]
    fn test_extract_module_names_no_modules_annotation() {
        let mut annotations = std::collections::BTreeMap::new();
        annotations.insert(
    ...
    #[test]
    fn test_extract_module_names_none_annotations() {
        let result = extract_module_names(None);

        assert!(result.is_empty());
    }

    #[test]
    fn test_is_valid_ingress_class_valid() {
        let spec = k8s_openapi::api::networking::v1::IngressSpec {
            ingress_class_name: Some(ksbh_core::constants::INGRESS_CLASS_NAME.to_string()),
            ..Default::default()
        };

        assert!(is_valid_ingress_class(&spec));
    }

    #[test]
    fn test_is_valid_ingress_class_invalid() {
        let spec = k8s_openapi::api::networking::v1::IngressSpec {
    ...
    #[test]
    fn test_is_valid_ingress_class_none() {
        let spec = k8s_openapi::api::networking::v1::IngressSpec {
    ...
    #[test]
    fn test_parse_tls_secret_from_string_data() {
        let mut secret = k8s_openapi::api::core::v1::Secret {
            string_data: Some(std::collections::BTreeMap::from([
                ("tls.crt".to_string(), "TEST_CERT".to_string()),
                ("tls.key".to_string(), "TEST_KEY".to_string()),
            ])),
            ..Default::default()
        };

        let result = parse_tls_secret(&mut secret, "test-secret");

        assert!(result.is_ok());
        let (crt, key) = result.unwrap();
        assert_eq!(crt, "TEST_CERT");
        assert_eq!(key, "TEST_KEY");
    }

    #[test]
    fn test_parse_tls_secret_missing_crt() {
        let mut secret = k8s_openapi::api::core::v1::Secret {
            string_data: Some(std::collections::BTreeMap::from([(
                "tls.key".to_string(),
                "TEST_KEY".to_string(),
        ...
        assert!(result.unwrap_err().contains("tls.crt"));
    }

    #[test]
    fn test_parse_tls_secret_missing_key() {
        let mut secret = k8s_openapi::api::core::v1::Secret {
            string_data: Some(std::collections::BTreeMap::from([(
                "tls.crt".to_string(),
                "TEST_CERT".to_string(),
